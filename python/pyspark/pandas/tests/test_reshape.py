...
class ReshapeTest(PandasOnSparkTestCase):
    ...
    def test_merge_asof(self):
        ...
        self.assert_eq(
            pd.merge_asof(pdf_left, pdf_right, on="a").sort_index(),
            ps.merge_asof(psdf_left, psdf_right, on="a").sort_index(),
        )
        self.assert_eq(
            pd.merge_asof(pdf_left, pdf_right, left_on="a", right_on="a").sort_index(),
            ps.merge_asof(psdf_left, psdf_right, left_on="a", right_on="a").sort_index(),
        )
        ...
        self.assert_eq(
            pd.merge_asof(pdf_left, pdf_right, on="a", by="b").sort_index(),
            ps.merge_asof(psdf_left, psdf_right, on="a", by="b").sort_index(),
        )
        self.assert_eq(
            pd.merge_asof(pdf_left, pdf_right, on="a", tolerance=1).sort_index(),
            ps.merge_asof(psdf_left, psdf_right, on="a", tolerance=1).sort_index(),
        )
        self.assert_eq(
            pd.merge_asof(pdf_left, pdf_right, on="a", allow_exact_matches=False).sort_index(),
            ps.merge_asof(psdf_left, psdf_right, on="a", allow_exact_matches=False).sort_index(),
        )
        self.assert_eq(
            pd.merge_asof(pdf_left, pdf_right, on="a", direction="forward").sort_index(),
            ps.merge_asof(psdf_left, psdf_right, on="a", direction="forward").sort_index(),
        )
        self.assert_eq(
            pd.merge_asof(pdf_left, pdf_right, on="a", direction="nearest").sort_index(),
            ps.merge_asof(psdf_left, psdf_right, on="a", direction="nearest").sort_index(),
        )
