...
class UDFLoggingTestsMixin:
    ...
    def test_udf_logging(self):
        ...
        for name, id, level, context in [
            ("udf1_1_id", udf1_1_id, "WARNING", {"udf_id": str(udf1_1_id), "a": "123"}),
            ("udf2_id", udf2_id, "INFO", {"udf_id": str(udf2_id), "b": "xyz"}),
            ("udf1_2_id", udf1_2_id, "WARNING", {"udf_id": str(udf1_2_id), "a": "123"}),
        ]:
    ...
    def test_udf_log_max_entries(self):
        ...
        self.assertEquals(self.spark.udf.logs.asDF().count(), 5)
        ...
        self.assertEquals(self.spark.udf.logs.asDF().count(), 10)

        # set to 2 without clean
        with self.sql_conf({"spark.sql.pyspark.udf.logging.maxEntries": 2}):
            df.collect()

        self.assertEquals(self.spark.udf.logs.asDF().count(), 2)
            ...
            assertDataFrameEqual(
                ...
                [
                    Row(
                        ...
                        logger="PySparkUDFLogger",
                        msg="This is a log message: 0",
                        context={"udf_id": str(udf1_id), "a": "123"},
                    ),
                    Row(
                        level="INFO",
                        logger="PySparkUDFLogger",
                        msg="This is a log message: 0",
                        context={"udf_id": str(udf2_id), "b": "xyz"},
                    ),
                ],
            )
