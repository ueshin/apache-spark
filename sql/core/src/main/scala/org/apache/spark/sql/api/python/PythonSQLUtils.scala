...
import org.apache.spark.storage.{BlockManagerMaster, PythonWorkerLogBlockId}
...
private[sql] object PythonSQLUtils extends Logging {
  ...
  def cleanupPythonWorkerLogs(sessionUUID: String): Unit = {
    cleanupPythonWorkerLogs(sessionUUID, SparkEnv.get.blockManager.master)
  }

  def cleanupPythonWorkerLogs(
      sessionUUID: String, blockManager: BlockManagerMaster): Unit = {
