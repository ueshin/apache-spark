          ...
          aggExpr.mode match {
            case Partial =>
              ...
              val udaf = PythonUDF(pythonUDAF.udaf.name, pythonUDAF.udaf.partial,
                pythonUDAF.udaf.bufferType, buffer.aggBufferAttributes,
                PythonEvalType.SQL_PANDAS_GROUP_AGGREGATE_UDF)
              udafs += udaf

              val (resultAttrs, replacingExprs) = pythonUDAF.inputAggBufferAttributes.map { attr =>
