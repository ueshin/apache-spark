        ...
        withPosition(u) {
          try {
            ...
            val tvf = resolvedFunc.transformAllExpressionsWithPruning(
              _.containsPattern(TABLE_ARGUMENT_EXPRESSION), ruleId)  {
              case t: FunctionTableRelationArgumentExpression =>
                val alias = s"__auto_generated_subquery_name_${tableArgs.size}"
                tableArgs.append(SubqueryAlias(alias, t.evaluable))
                UnresolvedAttribute(s"$alias.c")
            }
            if (tableArgs.nonEmpty) {
              val alias = s"__auto_generated_subquery_name"
              Project(
