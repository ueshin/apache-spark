...
class LogBlockWriterSuite extends SparkFunSuite {
  ...
  test("close resources when failed to initialize") {
  ...
  test("writeLog/save operations should fail on closed LogBlockWriter") {
    val logBlockWriter = makeLogBlockWriter()
  ...
  test("close writer after saving to block manager") {
    val log = LogLine(0L, 1, "Log message 1")
  ...
  test("skip saving to block manager if no logs written") {
    val logBlockWriter = spy(makeLogBlockWriter())
    logBlockWriter.save(TestLogBlockId(0L, "1"))
