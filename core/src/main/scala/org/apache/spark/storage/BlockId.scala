...
case class PythonWorkerLogBlockId(
  ...
  extends LogBlockId {
  override def name: String = {
    s"python_worker_log_${lastLogTime}_${executorId}_${workerId}_$sessionId"
  }
...
object BlockId {
  ...
  def apply(name: String): BlockId = name match {
    ...
    case PYTHON_WORKER_LOG_BLOCK(lastLogTime, executorId, workerId, sessionId) =>
      PythonWorkerLogBlockId(lastLogTime.toLong, executorId, workerId, sessionId)
